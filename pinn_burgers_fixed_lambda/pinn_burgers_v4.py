...
yi = torch.linspace(-1, 1, N, device=device).double()
...
# 'line_search_fn' ayuda a encontrar el tamaño de paso óptimo
optimizer_lbfgs = torch.optim.LBFGS(
    NNu.parameters(), 
    lr=1, 
    max_iter=50, # nº de iteraciones por paso
    history_size=50, 
...
def closure():
    ...
    loss = calculate_loss(False)
    
    loss.backward()
    return loss

# Bucle de L-BFGS
for i in range(30000): # L-BFGS suele converger en muy pocos pasos externos
    loss_lbfgs = optimizer_lbfgs.step(closure)
    
    if i % 1000 == 0:
        print(f"Epoch {i:5d} | Loss: {loss_lbfgs.item():.10e}")
            
    # Criterio de parada temprana si el error es ínfimo
    if loss_lbfgs < 1e-15:
        break
