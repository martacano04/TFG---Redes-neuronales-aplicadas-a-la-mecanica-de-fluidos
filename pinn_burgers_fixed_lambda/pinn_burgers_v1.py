...
NNu = NeuralNetwork().to(device)

def U(xi):
    return (NNu(xi) - NNu(-xi)) / 2

yi = torch.linspace(-1, 1, N, device=device)
yi = yi.view(-1, 1).requires_grad_(True)
yi_c = torch.tensor([[-2.0]], requires_grad=True).to(device)
...
yi_plot = torch.linspace(-L, L, 5000, device=device).view(-1, 1).requires_grad_(True)
