...
def plot_results(model, args, device, run_dir):
    ...
    f_der4 = get_grad(f_der3, yi_plot)
    f_der5 = get_grad(f_der4, yi_plot)
    ...
    fig.suptitle(rf"Resultados PINN ($\alpha\text{{ final}} = {lambda_:.5f}$)", fontsize=15)   
    ...
    # f'''(y)
    axs[1,1].plot(yi_plot.detach().cpu().numpy(), f_der5.detach().cpu().numpy())
    axs[1,1].set_title("Quinta Derivada del Residuo $f'''''(y)$")
